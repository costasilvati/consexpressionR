...
consexpressionR <- function() {
    ...
    shiny::fluidRow(
      ...
      shiny::fluidRow(
        ...
        shiny::column(
          width = 10,
          shiny::wellPanel(
            shiny::h3("Experiment Design", style = "color: #5e63b6;"),
            shiny::fluidRow(
              shiny::column(
                width = 6,
                shiny::textInput(
                  inputId = "experimentNameInp",
            ...
            shiny::fluidRow(
              ...
              shiny::column(
                ...
                shiny::conditionalPanel(
                  ...
                  shiny::radioButtons(
                    inputId = "condition1Inp", "Select first condition to be compared by the differential expression algorithm:",
                    choices = c(""), inline = TRUE
                  ),
                  shiny::radioButtons(
                    inputId = "condition2Inp", "Select second condition to be compared by the differential expression algorithm:",
                    choices = c(""), inline = TRUE
                  )
                ),
              )
      ...
      shiny::column(
        ...
        shiny::fluidRow(
          ...
          shiny::column(
            width = 3,
            shiny::wellPanel(
              class = "well-panel-tools",
              shiny::h3("limma", style = "color: #5e63b6;"),
              shiny::selectInput(
                inputId = "methNormLimmaInp",
              ...
              shiny::selectInput(
                inputId = "adjPvalueLimma",
                label = "Method to adjust P-Values",
                choices = c("holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr", "none"),
                selected = "BH"
              ),
              shiny::numericInput(
                inputId = "topTableLimmaInp",
              ...
              shiny::conditionalPanel(
                condition = "input.groupNameInp.split(',').filter(function(e){ return e.trim().length > 0 }).length <= 2",
                shiny::numericInput(
                  inputId = "lfcMinLimmaInp",
                  label = "Log Fold Change less or equal to",
                  value = -2.0,
                  step = 2
                ),
                shiny::numericInput(
                  inputId = "lfcMaxLimmaInp",
          ...
          shiny::column(
            ...
            shiny::wellPanel(
              ...
              shiny::numericInput(
                inputId = "lfcMinSamseqInp",
                label = "Log Fold Change less or equal to",
                value = -2.0,
                step = 2
              ),
              shiny::numericInput(
                inputId = "lfcMaxSamseqInp",
                label = "Log Fold Change greater or equal to",
                value = 2.0,
                step = 2
              ),
              shiny::numericInput(
                inputId = "qValueSamseqInp",
                value = 0.8,
                max = 1.0,
                min = 0.0,
          ...
          shiny::column(
            ...
            shiny::wellPanel(
              ...
              shiny::selectInput(
                inputId = "controlDeseq2Inp",
                choices = c(""),
                label = "Reference level (control sample)"
              ),
              shiny::h4("Differential Expression Metrics", class = "spacey"),
              shiny::numericInput(
                inputId = "lfcMinDeseq2Inp",
                label = "Log Fold Change less or equal to",
                value = -2.0,
                step = 2
              ),
              shiny::numericInput(
                inputId = "lfcMaxDeseq2Inp",
                label = "Log Fold Change greater or equal to",
                value = 2.0,
                step = 2
              ),
              shiny::numericInput(
                inputId = "pValueDeseq2Inp",
                value = 0.05,
                label = "Maximum P-value ",
                max = 1.0,
                min = 0.0,
          ...
          shiny::column(
            ...
            shiny::wellPanel(
              ...
              shiny::h4("Differential Expression Metrics", class = "spacey"),
              shiny::numericInput(
                inputId = "lfcMinEdgerInp",
                label = "Log Fold Change less or equal to",
                value = -2,
                step = 2
              ),
              shiny::numericInput(
                inputId = "lfcMaxEdgerInp",
                label = "Log Fold Change greater or equal to",
                value = 2,
                step = 2
              ),
              shiny::numericInput(
                inputId = "pValueEdgerInp",
                value = 0.05,
                label = "Maximum P-value ",
                max = 1.0,
                min = 0.0,
      ...
      shiny::column(
        ...
        shiny::fluidRow(
          ...
          shiny::column(
            width = 3,
            shiny::wellPanel(
              class = "well-panel-tools2",
              shiny::h3("NOISeq", style = "color: #5e63b6;"),
              shiny::selectInput(
                inputId = "methNormNoiseqInp",
                label = "Method to normalize library sizes",
          ...
          shiny::column(
            ...
            shiny::wellPanel(
              ...
              shiny::numericInput(
                inputId = "lfcMinKnowseqInp",
                label = "Log Fold Change less or equal to",
                value = -2,
                step = 2
              ),
              shiny::numericInput(
                inputId = "lfcMaxKnowseqInp",
                label = "Log Fold Change greater or equal to",
                value = 2,
                step = 2
              ),
              shiny::numericInput(
                inputId = "pValueKnowseqInp",
                value = 0.05,
                label = "Maximum P-value ",
                max = 1.0,
                min = 0.0,
  ...
  server <- function(input, output, session) {
    ...
    groupUpdate <- shiny::eventReactive(input$groupNameInp, {
      ...
      if (length(groups) > 2) {
        shiny::updateRadioButtons(
          inputId = "condition1Inp",
          choices = groups,
          inline = TRUE
        )
        shiny::updateRadioButtons(
          inputId = "condition2Inp",
          choices = groups,
          inline = TRUE
        )
    ...
    datasetCount <- shiny::eventReactive(input$go, {
      ...
      shiny::validate(
        shiny::need(informedColumns == colsReadData,
          label = paste(
            "ERROR: infromed ", qtdGroups, " groups of tretment and ",
    ...
    cons_res <- shiny::eventReactive(input$goDeg, {
      ...
      consResult$exp <- runExpression(
        numberReplics = input$numberReplicsInp,
    ...
    consensusPlot <- shiny::eventReactive(input$goUpsetPlot, {
      expDef_result$df <- expressionDefinition(
        resultTool = consResult$exp,
        groups = c(unlist(strsplit(input$groupNameInp, ","))),
      ...
      deByTool <- listDeByTool(
        consexpressionList = consResult$exp,
    ...
    deConsList <- shiny::eventReactive(input$goConsList, {
      ...
      remove(m)
      consListFinal$dfList <- consensusList(
        consexpressionList = consResult$exp,
        deTool = deByTool,
        threshold = input$thresoldDe
      )
      if (length(consListFinal$dfList) <= 0) {
        consListFinal$dfList <- paste(
          "No genes identified as differentially expressed by ",
          input$thresoldDe,
          " methods were found."
        )
      }
