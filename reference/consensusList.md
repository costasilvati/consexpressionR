...
``` r
consensusList(consexpressionList, deTool, threshold = 5)
```
...
- consexpressionList:
  ...
  An `ExpressionResultSet` object containing the results from each
  analysis method.

- deTool:

  data.frame. Output of 'listDeByTool' function

- threshold:

  Integer. Minimum number of tools that must agree on a gene being DE
  (default: 5)

## Value

An `ExpressionResultSet` object with the consensus slot populated
...
#> Average number of DEGs per method: 86.83
#> Number of genes in consensus: 6
```
